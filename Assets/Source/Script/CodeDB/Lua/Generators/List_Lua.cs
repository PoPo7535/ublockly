...
namespace UBlockly
{
    public partial class LuaGenerator
    {
        ...
        private CodeStruct List_GetIndex(Block block)
        {
            ...
            // If `dictionary` would be evaluated more than once (which is the case for LAST,
            ...
            {
                // Either `dictionary` is a simple variable, or we only need to refer to `dictionary` once.
        ...
        private string List_RemoveIndex(Block block)
        {
            ...
            if ((where == "LAST" || where == "FROM_END" || where == "RANDOM")
                && !regex.IsMatch(list))
            {
                // `dictionary` is an expression, so we may not evaluate it more than once.
    ...
    return t", GetListIndexStr("source", where1, "at1"), GetListIndexStr("source", where2, "at2")) +
        ...
        private CodeStruct List_Sort(Block block)
        {
            ...
            string funcName = Lua.Generator.ProvideFunction("list_sort",
                "function " + Generator.FUNCTION_NAME_PLACEHOLDER + @"(dictionary, typev, direction)
                     local t = {}
                     for n,v in pairs(dictionary) do table.insert(t, v) end
                     local compareFuncs = {
