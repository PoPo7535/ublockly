...
namespace UBlockly
{
    public class Workspace
    {
        ...
        public VariableMap ObjectMap { get; private set; }
        
        public enum MapType
        {
            Variable, Object
        }
        public VariableMap isMap(MapType mapType = MapType.Variable)
        {
            return mapType == MapType.Variable ? VariableMap : ObjectMap;
        }
        ...
        public Workspace(WorkspaceOptions options = null, string optId = null)
        {
            ...
            ObjectMap = new VariableMap(this);
        }
        ...
        {
            BlockDB.TryGetValue(id, out var block);
            return block;
        }

        /// <summary>
        /// Add a block to the dictionary of top blocks.
        /// </summary>
        ...
        /// <summary>
        /// Remove a block from the dictionary of top blocks.
        /// </summary>
        ...
        /// @param {boolean} ordered Sort the dictionary if true.
        ...
        public VariableModel CreateVariable(string name, string optType = null, string optId = null, MapType mapType = MapType.Variable)
        {
            return this.isMap(mapType).CreateVariable(name, optType, optId);
        }
        ...
        /// </summary>
        /// <param name="name"> The name to check for.</param>
        public VariableModel GetVariable(string name, MapType mapType = MapType.Variable)
        {
            return isMap(mapType).GetVariable(name);
        }
        ...
        public VariableModel GetVariableById(string id,MapType mapType)
        {
            return isMap(mapType).GetVariableById(id);
        }
        ...
        public List<VariableModel> GetVariablesOfType(string type,MapType mapType)
        {
            return this.isMap(mapType).GetVariablesOfType(type);
        }

        public List<string> GetVariableTypes (MapType mapType)
        {
            return isMap(mapType).GetVariableTypes();
        }
        
        public List<VariableModel> GetAllVariables (MapType mapType)
        {
            return isMap(mapType).GetAllVariables();
        }
        ...
        public void DeleteVariable(string name, MapType mapType)
        {
            ...
            var workspace = this;
            var variable = workspace.GetVariable(name,mapType);
            if (uses.Count > 1)
            {
                // Confirm before deleting multiple blocks.
                Debug.Log("confirm:" + I18n.Get(MsgDefine.DELETE_VARIABLE_CONFIRMATION).Replace("%1", uses.Count.ToString()).Replace("%2", name));
                workspace.DeleteVariableInternal(variable,mapType);
            }
            else
            {
                // No confirmation necessary for a single block.
                this.DeleteVariableInternal(variable,mapType);
            }
        }
        ...
        {
            var variable = this.GetVariableById(id,Workspace.MapType.Variable);
            if (null != variable)
            {
                this.DeleteVariableInternal(variable,Workspace.MapType.Variable);
            }
            else
            {
        ...
        public void DeleteVariableInternal(VariableModel variable, MapType mapType)
        {
            ...
            isMap(mapType).DeleteVariable(variable);
        }
        ...
        public void RenameVariableInternal(VariableModel variable, string newName, MapType mapType = MapType.Variable)
        {
            var newVariable = this.GetVariable(newName, mapType);
            ...
            this.isMap(mapType).RenameVariable(variable, newName);
        ...
        /// <param name="newName"> New variable name.</param>
        public void RenameVariable(string oldName, string newName, MapType mapType = MapType.Variable)
        {
            // Warning: Prefer to use renameVariableById.
            var variable = this.GetVariable(oldName,mapType);
            this.RenameVariableInternal(variable, newName,mapType);
        }
        ...
        // {
        //     // Warning: Prefer to use renameVariableById.
        //     var variable = this.GetObject(oldName);
        //     this.RenameObjectInternal(variable, newName);
        // }
        ...
        public void UpdateVariableStore(bool clear = false,List<string> unitTestAllUsedVariable = null, MapType mapType = MapType.Variable)
        {
            ...
            foreach (var name in variableNames)
            {
                ...
                var tempVar = GetVariable(name,mapType);
                if (null != tempVar)
                {
            ...
            if (clear) isMap(mapType).Clear();
            
            // Update the dictionary in place so that the flyout's references stay correct.
            foreach (var varDict in varList)
            {
                if (null == this.GetVariable(varDict["name"].ToString(),mapType))
                {
